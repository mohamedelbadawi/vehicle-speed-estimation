...
def calculate_optical_flow(cap, old_gray_frame, old_corners, centroid_old_row, centroid_old_col, c1, ans):
    ...
    while True:
        ...
        for index in range(len(new_corners)):
            if find_distance(new_corners[index][0][1], new_corners[index][0][0], int(centroid_row),
                             int(centroid_col)) > 30:
        ...
        for corner in new_corners_updated:
            ...
            if int(corner[0][0]) > global_variables.END_OF_THE_FRAME:
                flag = 1
                break
...
def code():
    ...
    while True:
        ...
        while cap.read()!=None:
            ...
            cv2.rectangle(frame, (global_variables.top_left[1], global_variables.top_left[0]),
                          (global_variables.bottom_right[1], global_variables.bottom_right[0]), color=(100, 255, 100),
                          thickness=4)
            cv2.rectangle(frame, (global_variables.top_left2[1], global_variables.top_left2[0]),
                          (global_variables.bottom_right2[1], global_variables.bottom_right2[0]), color=(100, 255, 100),
                          thickness=4)
            ...
            if int(fgmask[int(global_variables.top_left[0] + global_variables.bottom_right[0]) / 2, (global_variables.top_left[1] +global_variables.bottom_right[1]) / 2]) > 125:
                c += 1
                if c > 4:
                    calculate_optical_flow(cap, old_gray_frame, old_corners, centroid_old_row, centroid_old_col, c1,
                                           ans)
            elif int(fgmask[int(global_variables.top_left2[0] + global_variables.bottom_right2[0]) / 2, (
                                                                                                                global_variables.top_left2[
                                                                                                                    1] +
                                                                                                                global_variables.bottom_right2[
                                                                                                                    1]) / 2]) > 125:
                c2 += 1
                if c2 > 4:
                    calculate_optical_flow(cap, old_gray_frame2, old_corners2, centroid_old_row2, centroid_old_col2, c1,
                                           ans)

                    # Initialising Region of Interests for 2 lanes
            gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            roi = gray_frame[global_variables.top_left[0]:global_variables.bottom_right[0],
                  global_variables.top_left[1]:global_variables.bottom_right[1]]  # selecting roi
            new_corners = cv2.goodFeaturesToTrack(roi, 50, 0.01, 10)
            roi2 = gray_frame[global_variables.top_left2[0]:global_variables.bottom_right2[0],
                   global_variables.top_left2[1]:global_variables.bottom_right2[1]]  # selecting roi
